...
describe("lookup_barcode", () => {
    test("OFF outage records success:false with a category and no isError", async () => {
        const originalUa = process.env.OFF_USER_AGENT;
        delete process.env.OFF_USER_AGENT;
        try {
            await withTools(null, async (call) => {
                const r = await call("lookup_barcode", {
                    barcode: "737628064502",
                });
                expect(r.isError).not.toBe(true);
                const text = textOf(r);
                expect(text).toContain("Couldn't reach Open Food Facts");
                expect(text).toContain("Estimate");
            });
            expect(db.analyticsRows).toHaveLength(1);
            expect(db.analyticsRows[0]).toMatchObject({
                tool_name: "lookup_barcode",
                success: false,
                error_category: "service_misconfigured",
            });
        } finally {
            if (originalUa === undefined) delete process.env.OFF_USER_AGENT;
            else process.env.OFF_USER_AGENT = originalUa;
        }
    });
});
